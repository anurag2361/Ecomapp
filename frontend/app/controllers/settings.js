...
module.exports.controller = function (app) {
                ...
                userModel.findOne({ 'userId': req.params.userId }, function (err, newResult) {
                    ...
                    if (err) {
                        ...
                        res.render('message',
                            {
                                title: "Error",
                                msg: "Data Updated with Some Error",
                                status: 500,
                                error: err,
                                user: req.session.user,
                                cart: req.session.cart
    ...
    router.put("/change/password/:userId", auth.checkLogin, function (req, res) {
        ...
        var oldePass = encrypt.encryptPassword(req.body.oldPassword);
        ...
        userModel.findOne({ $and: [{ 'userId': req.params.userId }, { 'password': oldePass }] }, function (err, result) {
            if (err) {
                console.log(err);
                res.render('message',
                    {
